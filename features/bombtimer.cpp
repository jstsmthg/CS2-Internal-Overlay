...
static float ReadCurTimeImpl(uintptr_t clientBase) {
  __try {
    ...
    if (globalVarsPtr && globalVarsPtr > 0x10000) {
...
// Use GameRules as the authoritative source of truth for bomb state
static bool IsBombPlantedViaGameRules(uintptr_t clientBase) {
  __try {
    uintptr_t gameRulesPtr = *(uintptr_t *)(clientBase + offsets::dwGameRules);
    if (!gameRulesPtr || gameRulesPtr < 0x10000)
      return false;
    // C_CSGameRules::m_bBombPlanted = 0x8C7
    bool planted = *(bool *)(gameRulesPtr + 0x8C7);
    return planted;
  } __except (EXCEPTION_EXECUTE_HANDLER) {
    return false;
  }
...
uintptr_t bombtimer::FindPlantedC4Entity() {
  ...
  // GATE: Only proceed if GameRules says bomb is planted
  if (!IsBombPlantedViaGameRules(clientBase))
    return 0;
  ...
  __try {
    ...
    // Validate: bomb must be ticking
    bool ticking = *(bool *)(entity + schemas::C_PlantedC4::m_bBombTicking);
    if (!ticking)
      return 0;

    // Validate: blow time must be positive and reasonable
    float blowTime = *(float *)(entity + schemas::C_PlantedC4::m_flC4Blow);
    if (blowTime <= 0.0f || blowTime > 1000000.0f)
      return 0;

    // Validate: timer length should be 40s (competitive) or at least > 0
    float timerLength = *(float *)(entity + schemas::C_PlantedC4::m_flTimerLength);
    if (timerLength <= 0.0f || timerLength > 120.0f)
      return 0;

    // Validate: bomb site must be 0 (A) or 1 (B)
    int bombSite = *(int *)(entity + schemas::C_PlantedC4::m_nBombSite);
...
void bombtimer::Render() {
  ...
  __try {
    // Re-read and validate all values inside SEH
    ...
    // Use actual timer length for validation instead of hardcoded 45
    float timerLength = *(float *)(c4Entity + schemas::C_PlantedC4::m_flTimerLength);
    if (timerLength <= 0.0f)
      timerLength = 40.0f;

    if (remaining <= 0.f || remaining > timerLength + 2.0f)
      return;
    ...
    // Sanity check defuse remaining
    if (defuseRemaining < 0.0f || defuseRemaining > 15.0f)
      defuseRemaining = 0.0f;
    ...
    if (remaining < 5.f)
      timerColor = IM_COL32(255, 0, 0, 255);
    else if (remaining < 10.f)
      timerColor = IM_COL32(255, 165, 0, 255);

    float barWidth = 260.f, barHeight = 20.f;
    float barLeft = centerX - barWidth * 0.5f;
    float barFrac = remaining / timerLength;
    if (barFrac > 1.f)
      barFrac = 1.f;
    if (barFrac < 0.f)
      barFrac = 0.f;
    ...
    if (beingDefused && defuseRemaining > 0.0f) {
      float defuseLength =
          *(float *)(c4Entity + schemas::C_PlantedC4::m_flDefuseLength);
      if (defuseLength <= 0.0f || defuseLength > 15.0f)
        defuseLength = 10.0f;

      float defuseFrac = defuseRemaining / defuseLength;
      if (defuseFrac < 0.0f)
        defuseFrac = 0.0f;
      if (defuseFrac > 1.0f)
        defuseFrac = 1.0f;
    ...
    if (hooks::bombDefuseCircle) {
      ...
      if (sceneNode && sceneNode > 0x10000) {
        ...
        if (WorldToScreen(origin, viewMatrix, displaySize.x, displaySize.y, screen)) {
          float explodeArc = remaining / timerLength;
          if (explodeArc < 0.0f)
            explodeArc = 0.0f;
          if (explodeArc > 1.0f)
            explodeArc = 1.0f;
          ...
          if (beingDefused && defuseRemaining > 0.0f) {
            float defuseLength2 =
                *(float *)(c4Entity + schemas::C_PlantedC4::m_flDefuseLength);
            if (defuseLength2 <= 0.0f || defuseLength2 > 15.0f)
              defuseLength2 = 10.0f;

            float defuseArc = defuseRemaining / defuseLength2;
            if (defuseArc < 0.0f)
              defuseArc = 0.0f;
            if (defuseArc > 1.0f)
              defuseArc = 1.0f;
    ...
    // Silently catch any access violation from stale entity memory
  }
}
